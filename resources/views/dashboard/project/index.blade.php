                    ...
                    <form action="{{ route('dashboard.project.store') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group form-group col-md-4">
                            ...
                            <select name="file_type" class="form-control" id="file_type">
                                <option value="image" {{ $project?->file_type == 'image' ? 'selected' : '' }}>Image</option>
                                <option value="video" {{ $project?->file_type == 'video' ? 'selected' : '' }}>Video</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-12">
                            @if($project?->file_type == 'video')
                                <video class="form-group col-md-6" controls style="width: 100px;">
                                    <source src="{{ $project?->media[0]->original_url }}" type="" class="img-thumbnail image-preview">
                                </video>
                            @else
                                <img src="{{  $project?->media[0]->original_url  }}" style="width: 100px" class="img-thumbnail image-preview" alt="">
                            @endif
                        </div>
