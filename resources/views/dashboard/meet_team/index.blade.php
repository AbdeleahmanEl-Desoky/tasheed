        ...
        <section class="content">
            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <form id="upload-form" action="{{ route('dashboard.meet_team.store') }}" method="post" enctype="multipart/form-data">

                        {{ csrf_field() }}
                        {{ method_field('post') }}

                            <input type="hidden" name="id" class="form-control" value="{{ $meet?->id }}">

                        <div class="form-group col-md-12">
                            <label>@lang('site.title')</label>
                            <input type="text" name="title" class="form-control" value="{{ $meet?->title }}">
                        </div>
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control ckeditor" id="ex1"
                                      name="description" rows="3">{{$meet?->description}}</textarea>
                        </div>
                        ...
                        <div class="form-group form-group col-md-4">
                            ...
                            <select name="file_type" class="form-control" id="file_type">
                                <option value="image" {{ $meet?->file_type == 'image' ? 'selected' : '' }}>Image</option>
                                <option value="video" {{ $meet?->file_type == 'video' ? 'selected' : '' }}>Video</option>
                            </select>
                        </div>
                        ...
                        <hr>
                        <div class="form-group col-md-12">
                            @if($meet?->file_type == 'video')
                                <video class="form-group col-md-6" controls style="width: 100px;">
                                    <source src="{{ $meet?->media[0]->original_url }}" type="" class="img-thumbnail image-preview">
                                </video>
                            @else
                                <img src="{{  $meet?->media[0]->original_url  }}" style="width: 100px" class="img-thumbnail image-preview" alt="">
                            @endif
                        </div>
                        <div class="form-group col-md-12">
    ...
    document.getElementById('upload-button').addEventListener('click', function (e) {
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                ...
                window.location.href = "{{ route('dashboard.meet_team.index') }}"; // Redirect on success
            } else {
